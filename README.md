...
> **publish 不会影响你正在使用的浏览器。** 发布时会在 Edge 中打开独立的 tab（每个平台一个），只在那些 tab 里自动操作——填表、点击、保存草稿。你的 gaoshi 前端 tab 和其他浏览页面完全不受影响，鼠标光标也不会被占用。各平台创作者中心（抖音/B站/小红书）需在 Edge 中手动登录一次，后续 publish 自动复用登录 session。
