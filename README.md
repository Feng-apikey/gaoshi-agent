...
> 稿事只做到草稿箱。
> 我们不替你点发布——这一步永远留给你自己。
> 你的账号 cookies、草稿、图片、登录态，全部留在你本机 Edge 浏览器里。
> 我们没有云端、没有账号系统、没有用户数据——这不是免责，是我们主动选择的产品边界。
...
3. 编译 gaoshi-mcp
4. 构建前端
5. 初始化运行时目录（`data/`、`cache/`）
6. 从模板初始化记忆文件

> 稿事通过 Playwright 连接本地 Edge CDP（`--remote-debugging-port=9222`），无需额外下载 Chromium。
...
> **发布期间不要碰 publish 正在操作的 tab。** 它会自动打开平台创作者页面，在里面模拟拟人操作（鼠标移动、点击、填表）。如果你在同一 tab 里点击或输入，两个"人"抢一个页面，拟人化会暴露，发布也会中断。其他 tab 随便用。各平台创作者中心（抖音/B站/小红书）需在 Edge 中手动登录一次，后续 publish 自动复用登录 session。
...
1. 确认 Edge 已启动并开启了调试端口：`start msedge --remote-debugging-port=9222`
2. 确认各平台创作者中心在 Edge 中已手动登录过一次
...
│
├── publish/                  # 平台发布引擎（Playwright + Edge CDP）
│   ├── index.ts              # 发布路由 + 素材/标签校验
│   ├── browser-manager.ts    # CDP 连接管理（复用 Edge session）
│   ├── douyin.ts             # 抖音发布（图文/视频/长文，长文标签需手动添加话题）
│   ├── bilibili.ts           # B站发布（动态/视频/专栏）
│   ├── xiaohongshu.ts        # 小红书发布（图文/视频/长文）
│   ├── helpers.ts            # 填表/点击/上传（PowerShell SendKeys 调 OS 文件对话框）
│   ├── humanize.ts           # 拟人化（鼠标轨迹/粘贴/空闲抖动）
│   └── types.ts              # DraftData 类型
│
├── tools/                    # MCP 服务
...
- 同一账号每天发布次数建议 ≤ 5，间隔 ≥ 30 分钟
- 内容请遵守各平台 ToS 和当地法规
- 不建议在多账号矩阵场景下使用（平台会聚类设备指纹）
