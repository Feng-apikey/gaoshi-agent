...
> **关于拟人化操作：** 发布时浏览器会自动打开平台页面、填表、保存草稿——鼠标会自动移动和点击。这些操作通过 CDP 协议直连浏览器引擎，**不会占用你的真实鼠标光标**，你可以在同一个 Edge 里正常浏览其他页面，互不干扰。各平台创作者中心（抖音/B站/小红书）需在 Edge 中手动登录一次，后续 publish 自动复用登录 session。
